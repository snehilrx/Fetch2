...
const val VIDEO_CACHE = "cache/videos"
...
actual fun VideoPlayerImpl(
    ...
    if (isHlsVideo(url)) {
        // first download the file then play
        var isFirstSegmentFetched by remember { mutableStateOf(false) }
        val outputFilePath = "$VIDEO_CACHE/${slug}.mp4"
        LaunchedEffect(isFirstSegmentFetched) {
            HlsHelper.downloadHLS(url, outputFilePath) {
                isFirstSegmentFetched = true
            }
        }
        if (isFirstSegmentFetched) {
            VideoPlayerImplInternal(progressState, onFinish, "file://${File(outputFilePath).absolutePath}", seek, speed, volume, isResumed, isFullscreen, modifier)
        }
    } else {
        VideoPlayerImplInternal(progressState, onFinish, url, seek, speed, volume, isResumed, isFullscreen, modifier)
    }
}

fun isHlsVideo(url: String) = url.endsWith("m3u8")

@Composable
private fun VideoPlayerImplInternal(
    progressState: MutableState<Progress>,
    onFinish: (() -> Unit)?,
    url: String,
    seek: Float,
